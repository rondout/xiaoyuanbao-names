...
export const NAME_STORAGE_KEY = 'names'

export const saveNamesToStorage = (names: string[]) => {
    localStorage.setItem(NAME_STORAGE_KEY, JSON.stringify(names))
}

export const getNamesFromStorage = (): string[] => {
    try {
...
export function getAllNames(gender: Genders) {
    const names = new NamesFactory(gender)
    console.log(names);
    // @ts-ignore
    window.a = names
